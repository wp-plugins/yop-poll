    ...
    class YOP_POLL_General_Options extends YOP_POLL_Abstract_Admin {
        ...
        public function general_options_validate( $input ) {
            ...
            if( current_user_can( 'manage_yop_polls_options' ) ) {
                ...
                if( isset ( $input ['user_interface_type'] ) ) {
                    if( in_array( $input ['user_interface_type'], array( 'advanced' ) ) ) {
                        if( $default_options ['user_interface_type'] != trim( $input ['user_interface_type'] ) ) {
                            $newinput ['user_interface_type'] = trim( $input ['user_interface_type'] );
                            $updated .= __yop_poll( 'Option "User Interface Type" Updated!' ) . $message_delimiter;
                        }
                    }
                    else {
                        $newinput ['user_interface_type'] = $default_options ['user_interface_type'];
                        $errors .= __yop_poll( 'Option "User Interface Type" Not Updated! Please choose between \'Beginner\', \'Advanced\'' ) . $message_delimiter;
                    }
                }
                //</editor-fold>
                ...
                if( isset( $input['poll_start_date'] ) ) {
                    if( '' != $input['poll_start_date'] ) {
                        if( $default_options ['poll_start_date'] != trim( $input ['poll_start_date'] ) ) {
                            $newinput ['poll_start_date'] = convert_date( trim( $input ['poll_start_date'] ), "d-m-Y h:i:s" );
                        ...
                        $errors .= __yop_poll( 'Option "Poll Start Date" Not Updated! The field is empty!' ) . $message_delimiter;
                ...
                if( isset( $input['poll_never_expire'] ) && 'yes' == $input['poll_never_expire'] ) {
                    if( $default_options['poll_end_date'] != $input['poll_end_date'] ) {
                        $newinput['poll_end_date'] = "18-01-2038 23:59:59";
                    ...
                    if( isset( $input['poll_end_date'] ) ) {
                        if( '' != $input['poll_end_date'] ) {
                            if( $default_options['poll_end_date'] != $input['poll_end_date'] ) {
                                $newinput ['poll_end_date'] = convert_date( trim( $input ['poll_end_date'] ), "d-m-Y h:i:s" );;
                                $updated .= __yop_poll( 'Option "Poll End Date" Updated!' ) . $message_delimiter;
                            }
                        }
                        else {
                            $newinput ['poll_end_date'] = $default_options ['poll_end_date'];
                            $errors .= __yop_poll( 'Option "Poll End Date" Not Updated! The field is empty!' ) . $message_delimiter;
                        }
                    }
                    else {
                        $newinput ['poll_end_date'] = $default_options ['poll_end_date'];
                    ...
                    if( count( array_intersect( array(
                        ...
                        if( in_array( 'custom-date', $newinput ['view_results'] ) ) {
                            if( isset ( $input ['view_results_start_date'] ) ) {
                                ...
                                $updated .= __yop_poll( 'Option "View Results Custom Date" Not Updated! Field is empty' ) . $message_delimiter;
                            }
                        }
                    }
                    else {
                        $newinput ['view_results'] = $default_options ['view_results'];
                        $errors .= __yop_poll( 'Option "View Results" Not Updated! Please choose one of: \'Before\', \'After\', \'After Poll End Date\', \'Never\' or \'Custom Date\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_results'] = $default_options ['view_results'];
                ...
                if( isset ( $input ['view_results_permissions'] ) ) {
                    if( count( array_intersect( array(
                        ...
                        $errors .= __yop_poll( 'Option "View Results Permissions" Not Updated! Please choose between \'Quest Only\', \'Registered Only\', \'Guest & Registered Users\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_results_permissions'] = $default_options ['view_results_permissions'];
                ...
                if( isset ( $input ['view_results_type'] ) ) {
                    if( in_array( $input ['view_results_type'], array(
                        ...
                        $errors .= __yop_poll( 'Option "View Results Type" Not Updated! Please choose between: \'Votes number\', \'Percentages\' or \'Votes number and percentages\' ' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_results_type'] = $default_options ['view_results_type'];
                ...
                if( isset ( $input ['answer_result_label'] ) ) {
                    if( 'votes-number' == $input ['view_results_type'] ) {
                        if( stripos( $input ['answer_result_label'], '%POLL-ANSWER-RESULT-VOTES%' ) === false ) {
                            ...
                            $errors .= __yop_poll( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-VOTES%!' ) . $message_delimiter;
                        }
                        else {
                    ...
                    if( 'percentages' == $input ['view_results_type'] ) {
                        if( stripos( $input ['answer_result_label'], '%POLL-ANSWER-RESULT-PERCENTAGES%' ) === false ) {
                            $newinput ['answer_result_label'] = $default_options ['answer_result_label'];
                            $errors .= __yop_poll( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-PERCENTAGES%!' ) . $message_delimiter;
                        }
                        else {
                            if( $default_options ['answer_result_label'] != trim( $input ['answer_result_label'] ) ) {
                                $newinput ['answer_result_label'] = trim( $input ['answer_result_label'] );
                    ...
                    if( 'votes-number-and-percentages' == $input ['view_results_type'] ) {
                        if( stripos( $input ['answer_result_label'], '%POLL-ANSWER-RESULT-PERCENTAGES%' ) === false ) {
                            $newinput ['answer_result_label'] = $default_options ['answer_result_label'];
                            $errors .= __yop_poll( 'Option "Poll Answer Result Label" Not Updated! You must use %POLL-ANSWER-RESULT-VOTES% and %POLL-ANSWER-RESULT-PERCENTAGES%!' ) . $message_delimiter;
                        }
                    ...
                    $newinput ['answer_result_label'] = $default_options ['answer_result_label'];
                    $errors .= __yop_poll( 'Option "Poll Answer Result Label" Not Updated! Please fill in this field' ) . $message_delimiter;
                }
                ...
                if( isset ( $input ['vote_button_label'] ) ) {
                    if( '' != $input ['vote_button_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Vote Button Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['vote_button_label'] = $default_options ['vote_button_label'];
                ...
                if( isset ( $input ['view_results_link'] ) ) {
                    if( in_array( $input ['view_results_link'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Results Link" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_results_link'] = $default_options ['view_results_link'];
                ...
                if( isset ( $input ['view_results_link_label'] ) ) {
                    if( '' != $input ['view_results_link_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Results Link Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_results_link_label'] = $default_options ['view_results_link_label'];
                ...
                if( isset ( $input ['view_back_to_vote_link'] ) ) {
                    if( in_array( $input ['view_back_to_vote_link'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Back to Vote Link" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_back_to_vote_link'] = $default_options ['view_back_to_vote_link'];
                ...
                if( isset ( $input ['view_back_to_vote_link_label'] ) ) {
                    if( '' != $input ['view_back_to_vote_link_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Back to Vote Link Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_back_to_vote_link_label'] = $default_options ['view_back_to_vote_link_label'];
                ...
                if( isset ( $input ['view_total_votes'] ) ) {
                    if( in_array( $input ['view_total_votes'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Total Votes" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_total_votes'] = $default_options ['view_total_votes'];
                ...
                if( isset ( $input ['view_total_votes_label'] ) ) {
                    if( stripos( $input ['view_total_votes_label'], '%POLL-TOTAL-VOTES%' ) === false ) {
                        ...
                        $errors .= __yop_poll( 'You must use %POLL-TOTAL-VOTES% to define your Total Votes label!' ) . $message_delimiter;
                    }
                    else {
                ...
                if( isset ( $input ['view_total_answers'] ) ) {
                    if( in_array( $input ['view_total_answers'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Total Answers" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_total_answers'] = $default_options ['view_total_answers'];
                ...
                if( isset ( $input ['view_total_answers_label'] ) ) {
                    if( stripos( $input ['view_total_answers_label'], '%POLL-TOTAL-ANSWERS%' ) === false ) {
                        ...
                        $errors .= __yop_poll( 'You must use %POLL-TOTAL-ANSWERS% to define your Total Answers label!' ) . $message_delimiter;
                    }
                    else {
                ...
                if( isset ( $input ['vote_permisions'] ) ) {
                    if( count( array_intersect( array(
                        ...
                        $errors .= __yop_poll( 'Option "Vote Permissions" Not Updated! Please choose between \'Guest\' and \'Registered\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['vote_permisions'] = $default_options ['vote_permisions'];
                ...
                if( isset( $input['vote_permisions_wordpress'] ) && in_array( $input['vote_permisions_wordpress'], array(
                    ...
                    $errors .= __yop_poll( 'Option "Vote as Wordpress User" not updated! Please choose between \'yes\' and \'no\'' ) . $message_delimiter;
                }
                ...
                if( isset( $input ['vote_permisions_wordpress_label'] ) && '' != trim( $input ['vote_permisions_wordpress_label'] ) ) {
                    ...
                    $errors .= __yop_poll( 'Option "Vote as Wordpress User Buton Label" not updated! The field is empty' ) . $message_delimiter;
                }
                //</editor-fold>
                ...
                if( isset( $input['vote_permisions_anonymous'] ) && in_array( $input['vote_permisions_anonymous'], array(
                    ...
                    $errors .= __yop_poll( 'Option "Vote as Anonymous User" not updated! Please choose between \'yes\' and \'no\'' ) . $message_delimiter;
                }
                ...
                if( isset( $input ['vote_permisions_anonymous_label'] ) && '' != trim( $input ['vote_permisions_anonymous_label'] ) ) {
                    ...
                    $errors .= __yop_poll( 'Option "Vote as Anonymous User Buton Label" not updated! The field is empty' ) . $message_delimiter;
                }
                //</editor-fold>
                ...
                if( isset ( $input ['blocking_voters'] ) ) {
                    if( count( array_intersect( array(
                        ...
                        if( ! in_array( 'dont-block', $newinput ['blocking_voters'] ) ) {
                            ...
                            if( isset ( $input ['blocking_voters_interval_value'] ) ) {
                                if( ctype_digit( $input ['blocking_voters_interval_value'] ) ) {
                                    ...
                                    $errors .= __yop_poll( 'Option "Blocking Voters Interval Value" Not Updated! Please fill in a number!' ) . $message_delimiter;
                                }
                            }
                            ...
                            if( isset ( $input ['blocking_voters_interval_unit'] ) ) {
                                if( in_array( $input ['blocking_voters_interval_unit'], array(
                                    ...
                                    $errors .= __yop_poll( 'Option "Blocking Voters Interval Unit" Not Updated! Please choose between \'Seconds\', \'Minutes\', \'Hours\' or \'Days\'' ) . $message_delimiter;
                                }
                            }
                        }
                    }
                        ...
                        $errors .= __yop_poll( 'Option "Blocking Voters" Not Updated! Please choose between: \'Don`t Block\', \'Cookie\', \'Ip\', \'Username\', \'Cookie and Ip\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['blocking_voters'] = $default_options ['blocking_voters'];
                    $errors .= __yop_poll( 'Option "Blocking Voters" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                }
                        ...
                        if( 'yes' == $input ['limit_number_of_votes_per_user'] ) {
                            if( isset ( $input ['number_of_votes_per_user'] ) ) {
                                if( intval( $input ['number_of_votes_per_user'] ) <= 0 ) {
                                    ...
                                    $errors .= __yop_poll( '"Number of Votes per User" must be a number > 0 !' ) . $message_delimiter;
                                }
                                else {
                                ...
                                $newinput ['number_of_votes_per_user'] = $default_options ['number_of_votes_per_user'];
                                $errors .= __yop_poll( '"Number of Votes per User" Not updated! It must be a number > 0 !' ) . $message_delimiter;
                            }
                        }
                    }
                    else {
                        $newinput ['limit_number_of_votes_per_user'] = $default_options ['limit_number_of_votes_per_user'];
                        $errors .= __yop_poll( 'Option "Number of Votes per User" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['limit_number_of_votes_per_user'] = $default_options ['limit_number_of_votes_per_user'];
                ...
                if( isset ( $input ['percentages_decimals'] ) && '' != trim( $input ['percentages_decimals'] ) ) {
                    if( ctype_digit( $input ['percentages_decimals'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Percentages Decimals" Not Updated! Please fill in a number!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['percentages_decimals'] = $default_options ['percentages_decimals'];
                ...
                if( isset ( $input ['use_default_loading_image'] ) && in_array( $input ['use_default_loading_image'], array( 'yes',
                    ...
                    if( 'no' == $input ['use_default_loading_image'] ) {
                        if( isset ( $input ['loading_image_url'] ) ) {
                            if( stripos( $input ['loading_image_url'], 'http' ) === false ) {
                                ...
                                $errors .= __yop_poll( 'Option "Loading Image Url" Not updated! You must use a url like "http://.." to define your Loading Image Url!' ) . $message_delimiter;
                            }
                            else {
                    ...
                    $errors .= __yop_poll( 'Option "Use Default Loading Image" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                }
                    ...
                    if( in_array( $input ['redirect_after_vote'], array( 'yes', 'no' ) ) ) {
                        ...
                        if( 'yes' == $input ['redirect_after_vote'] ) {
                            ...
                            if( isset ( $input ['redirect_after_vote_url'] ) && '' != trim( $input ['redirect_after_vote_url'] ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Redirect After Vote Url" Not Updated! Please fill in an url!' ) . $message_delimiter;
                            }
                        }
                    }
                    else {
                        $newinput ['redirect_after_vote'] = $default_options ['redirect_after_vote'];
                        $errors .= __yop_poll( 'Option "Redirect After Vote" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['redirect_after_vote'] = $default_options ['redirect_after_vote'];
                ...
                if( isset ( $input ['date_format'] ) && '' != trim( $input ['date_format'] ) ) {
                    ...
                    $newinput ['date_format'] = $default_options ['date_format'];
                    $errors .= __yop_poll( 'Option "Poll Date Format" Not Updated! The field is empty' ) . $message_delimiter;
                }
                ...
                if( isset ( $input ['view_poll_archive_link'] ) ) {
                    if( in_array( $input ['view_poll_archive_link'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "View Poll Archive Link" Not Updated! Please choose between \'yes\' or \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['view_poll_archive_link'] = $default_options ['view_poll_archive_link'];
                ...
                if( isset ( $input ['auto_generate_poll_page'] ) ) {
                    if( in_array( $input['auto_generate_poll_page'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Auto Generate Poll Page" Not Updated! Please choose between \'yes\' and \'no\'' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['auto_generate_poll_page'] = $default_options ['auto_generate_poll_page'];
                ...
                if( isset ( $input ['use_captcha'] ) ) {
                    if( in_array( $input ['use_captcha'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Use CAPTCHA" Not Updated! Choose "yes" and "no"!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['use_captcha'] = $default_options ['use_captcha'];
                ...
                if( isset ( $input ['send_email_notifications'] ) ) {
                    if( in_array( $input ['send_email_notifications'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Send Email Notifications" Not Updated! Choose between "yes" and "no"!' ) . $message_delimiter;
                    }
                }
                else {
                    $newinput ['send_email_notifications'] = $default_options ['send_email_notifications'];
                ...
                if( isset ( $input ['allow_other_answers'] ) ) {
                    if( in_array( $input ['allow_other_answers'], array( 'yes', 'no' ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Allow Other Answer" Not Updated! Choose "yes" or "no"!' ) . $message_delimiter;
                    }
                    ...
                    if( 'yes' == $input ['allow_other_answers'] ) {
                        ...
                        if( isset( $input ['is_default_other_answer'] ) ) {
                            if( in_array( $input ['is_default_other_answer'], array( 'yes', 'no' ) ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Make \'Other answer\' default answer" Not Updated! Choose "yes" or "no"!' ) . $message_delimiter;
                            }
                        }
                        ...
                        if( isset ( $input ['add_other_answers_to_default_answers'] ) ) {
                            if( in_array( $input ['add_other_answers_to_default_answers'], array( 'yes', 'no' ) ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Add the values submitted in \'Other\' as answers" Not Updated! Choose "yes" or "no"!' ) . $message_delimiter;
                            }
                        }
                        ...
                        if( isset ( $input ['display_other_answers_values'] ) ) {
                            if( in_array( $input ['display_other_answers_values'], array( 'yes', 'no' ) ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Display Other Answers Values" Not Updated! Choose "yes" or "no"!' ) . $message_delimiter;
                            }
                        }
                    }
                }
                ...
                if( isset ( $input ['allow_multiple_answers'] ) ) {
                    if( in_array( $input ['allow_multiple_answers'], array( 'yes', 'no' ) ) ) {
                        ...
                        if( 'yes' == $input ['allow_multiple_answers'] ) {
                            if( isset ( $input ['allow_multiple_answers_number'] ) ) {
                                if( ctype_digit( $input ['allow_multiple_answers_number'] ) ) {
                                    ...
                                    $errors .= __yop_poll( 'Option "Max Number of allowed answers" Not Updated! Please fill in a number!' ) . $message_delimiter;
                                }
                            }
                            if( isset ( $input ['allow_multiple_answers_min_number'] ) ) {
                                if( ctype_digit( $input ['allow_multiple_answers_min_number'] ) ) {
                                    ...
                                    $errors .= __yop_poll( 'Option "Min Number of allowed answers" Not Updated! Please fill in a number!' ) . $message_delimiter;
                                }
                            }
                        }
                    }
                        ...
                        $errors .= __yop_poll( 'Option "Allow Multiple Answers" Not Updated! Choose "yes" or "no"!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['display_answers'] ) ) {
                    if( in_array( $input ['display_answers'], array( 'vertical', 'orizontal', 'tabulated' ) ) ) {
                        ...
                        if( 'tabulated' == $input ['display_answers'] ) {
                            ...
                            if( isset ( $input ['display_answers_tabulated_cols'] ) ) {
                                if( ctype_digit( $input ['display_answers_tabulated_cols'] ) ) {
                                    ...
                                    $errors .= __yop_poll( 'Option "Columns for Tabulated Display Answers" Not Updated! Please fill in a number!' ) . $message_delimiter;
                                }
                            }
                        }
                    }
                        ...
                        $errors .= __yop_poll( 'Option "Display Answers" Not Updated! you must choose between \'vertical\', \'horizontal\' or \'tabulated\'' ) . $message_delimiter;
                    }
                }
                    ...
                    if( in_array( $input ['sorting_results'], array(
                        ...
                        if( isset ( $input ['sorting_results_direction'] ) ) {
                            if( in_array( $input ['sorting_results_direction'], array( 'asc', 'desc' ) ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Sort Results Direction" Not Updated! Please choose between \'Ascending\' or \'Descending\'' ) . $message_delimiter;
                            }
                        }
                    }
                    else {
                        $newinput ['sorting_results'] = $default_options ['sorting_results'];
                        $errors .= __yop_poll( 'Option "Sort Results" Not Updated! Please choose between: \'as defined\', \'alphabetical\', \'database\' or \'votes\'' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['singular_answer_result_votes_number_label'] ) ) {
                    if( '' != $input ['singular_answer_result_votes_number_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Poll Answer Result Votes Number Singular Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['plural_answer_result_votes_number_label'] ) ) {
                    if( '' != $input ['plural_answer_result_votes_number_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Poll Answer Result Votes Number Plural Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['display_results'] ) ) {
                    if( in_array( $input ['display_results'], array( 'vertical', 'orizontal', 'tabulated' ) ) ) {
                        ...
                        if( 'tabulated' == $input ['display_results'] ) {
                            ...
                            if( isset ( $input ['display_results_tabulated_cols'] ) ) {
                                if( ctype_digit( $input ['display_results_tabulated_cols'] ) ) {
                                    ...
                                    $errors .= __yop_poll( 'Option "Columns for Tabulated Display Results" Not Updated! Please fill in a number!' ) . $message_delimiter;
                                }
                            }
                        }
                    }
                        ...
                        $errors .= __yop_poll( 'Option "Display Results" Not Updated! Choose the display layout: \'vertical\', \'horizontal\' or \'tabulated\'' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['bar_background'] ) ) {
                    if( ctype_alnum( $input ['bar_background'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Result Bar Background Color" Not Updated! Fill in an alphanumeric value!' ) . $message_delimiter;
                    }
                }
                if( isset ( $input ['bar_height'] ) ) {
                    if( ctype_digit( $input ['bar_height'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Result Bar Height" Not Updated! Please fill in a number!' ) . $message_delimiter;
                    }
                }
                if( isset ( $input ['bar_border_color'] ) ) {
                    if( ctype_alnum( $input ['bar_border_color'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Result Bar Border Color" Not Updated! Please fill in a number!' ) . $message_delimiter;
                    }
                }
                if( isset ( $input ['bar_border_width'] ) ) {
                    if( ctype_digit( $input ['bar_border_width'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Result Bar Border Width" Not Updated! Please fill in a number!' ) . $message_delimiter;
                    }
                }
                if( isset ( $input ['bar_border_style'] ) ) {
                    if( ctype_alpha( $input ['bar_border_style'] ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Result Bar Border Style" Not Updated! Fill in an alphanumeric value!' ) . $message_delimiter;
                    }
                }
                /* End Poll bar style*/
                /* End Results Options */
                    ...
                    if( in_array( $input['sorting_archive_polls'], array( 'as_defined', 'database', 'votes' ) ) ) {
                        ...
                        if( 'as_defined' != trim( $input['sorting_archive_polls'] ) ) {
                            if( isset( $input['sorting_archive_polls_rule'] ) ) {
                                ...
                                $errors .= __yop_poll( 'Option "Sort Archive Polls Rule!" Not Updated! Please choose between \'ascending\' or \'descending\'!' ) . $message_delimiter;
                            }
                        }
                    }
                    else {
                        $newinput ['sorting_archive_polls'] = $default_options ['sorting_archive_polls'];
                        $errors .= __yop_poll( 'Option "Sort Archive Polls" Not Updated! Please choose between \'as defined\' or \'database\' or \'votes\'' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['yop_poll_archive_page_id'] ) ) {
                    if( '' != $input ['yop_poll_archive_page_id'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Archive Page" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['archive_link_label'] ) ) {
                    if( '' != $input ['archive_link_label'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Poll Archive Link Label" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['archive_polls_per_page'] ) ) {
                    if( false != ( intval( trim( $input ['archive_polls_per_page'] ) ) ) ) {
                        ...
                        $errors .= __yop_poll( 'Option "Polls Per Page" Not Updated! Please fill in an integer!' ) . $message_delimiter;
                    }
                }
                /* End Archive Options */
                ...
                if( isset ( $input ['email_notifications_from_name'] ) ) {
                    if( '' != $input ['email_notifications_from_name'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Notifications From Name" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['email_notifications_from_email'] ) ) {
                    if( '' != $input ['email_notifications_from_email'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Notifications From Email" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['email_notifications_recipients'] ) ) {
                    if( '' != $input ['email_notifications_recipients'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Email Notifications Recipients" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['email_notifications_subject'] ) ) {
                    if( '' != $input ['email_notifications_subject'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Email Notifications Subject" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                ...
                if( isset ( $input ['email_notifications_body'] ) ) {
                    if( '' != $input ['email_notifications_body'] ) {
                        ...
                        $errors .= __yop_poll( 'Option "Email Notifications Body" Not Updated! The field is empty!' ) . $message_delimiter;
                    }
                }
                /* End Email Options */
            }
