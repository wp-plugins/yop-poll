    ...
    class Yop_Poll_DbSchema {
        ...
        public static function create_polls_table() {
            ...
            $optin_box_modal_options	= array(
                'show'					=> 'yes',
                'last_show_date'		=> current_time("mysql"),
                'modal_had_submit'		=> 'no',
...
function equalWidth_%POLL-ID%(obj, cols, toTabulate, column = 0 ) {
		for (var k in cols){ // get each question id
			if (cols.hasOwnProperty(k)) {
				var id = obj + k + toTabulate;
				var widest = 0;
				...
				width = parseInt( widest / parseInt( cols[k][column] ) );
...
function tabulate_answers_%POLL-ID%() {
	equalWidth_%POLL-ID%( "#yop-poll-container-%POLL-ID% #yop-poll-question-container-%POLL-ID%-", %ANSWERS-TABULATED-COLS%, ' .yop-poll-li-answer-%POLL-ID%', 0 );
}

function tabulate_results_%POLL-ID%() {
	equalWidth_%POLL-ID%( "#yop-poll-container-%POLL-ID% #yop-poll-question-container-%POLL-ID%-", %RESULTS-TABULATED-COLS%, ' .yop-poll-li-result-%POLL-ID%', 1 );
}
