...
class YOP_POLL_Pro_Admin extends YOP_POLL_Abstract_Admin {
            ...
            if( check_admin_referer( 'yop-poll-before-buy' ) ) {
...

                {
                    ...
                    if( isset( $response->status ) && $response->status == "200" ) {
                        ...
                        wp_die( "License Key is not valid" );
    ...
    public function do_buy() {
        if( isset( $_POST['do_buy'] ) && ($_POST['do_buy'] == "Buy now" || $_POST['do_buy']=="Upgrade to Pro for only $17") ) {
            if( check_admin_referer( 'yop-poll-before-buy' ) ) {
                //get domain name
                $domain = isset( $_SERVER['HTTP_HOST'] ) ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME'];
                ...
                if( '' != $domain ) {
                    ...
                    $paypalUrl    = "https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=UNBWZZ9SR7WXN&return=" .
                        "$redirect_url&rm=2&custom=" .
                        urlencode( $domain . ";" .
                            $rand_number
                        ) . "";
                    echo "<p>" . __yop_poll( "Please wait while you are being redirected to paypal. If you are not redirected within 10 seconds, please click " ) .
                        "<a href='$paypalUrl'>" . __yop_poll( "here" ) . "</a></p>";
                    echo "<script>window.location = '$paypalUrl';</script>";
                }
                else {
                    wp_die( __yop_poll( "There was an error determining your domain name. Please try again or contact support team at yop.ofice@gmail.com if problem persists!" ) );
                }
            }
        }
    }
