        ...
        if ( version_compare( $installed_version, '4.9.3', '<=' )||$x==1 ){
            if (!(version_compare(phpversion(), '5.3', '<'))) {
                ...
                $new_options=array(
                    ...
                    'sorting_results'                           => $current_options['sorting_results'],
                ...
                list($g1,$d)=split('-',$current_options['view_results_permissions']);
                $new_options['view_results_permissions' ]=array($g1,$d);
                list($g1,$d)=split('-',$current_options['view_results_link']);
        ...
        foreach( $polls as $poll ) {
                ...
                foreach( $current_options as $key => $value ) {
                    if( isset( $poll_option[$key] ) ) {
                        ...
                        if($key=='view_results_permissions'){
                            ...
                            list($g1,$d)=split('-',$poll_option[$key]);
                            if($g1=="quest"){
                                $g1="guest";
                            ...
                            if($key=='vote_permisions'){
                                list($g1,$d)=split('-',$poll_option['vote_permisions']);
                                if($g1=="quest"){
                                    $g1="guest";
            ...
            if( $poll_option['blocking_voters']=='cookie-ip')
                ...
                if( $poll_option['blocking_voters']=='username')
                    ...
                    $current_poll_meta['blocking_voters' ]=$poll_option['blocking_voters'];
            }
