    ...
    Class YOP_POLL_Poll_Model extends YOP_POLL_Abstract_Model {
        ...
        public static function return_template_preview_html( $template_id = '', $loc = 1 ) {
            if( '' == $template_id ) {
                ...
                $t = '<style type="text/css">' . self::return_poll_css( $template_details['css'], array(
                        "location"    => 'page',
                        'preview'     => true,
                        'template_id' => $uID,
                        'loc'         => $loc
            ...
            foreach( $this->questions as $question ) {
                ...
                if( isset( $question->allow_other_answers ) && 'yes' == $question->allow_other_answers ) {
                    if( isset( $question->display_other_answers_values ) && 'yes' == $question->display_other_answers_values ) {
                        $include_others  = true;
                        $display_answers = array( 'text', 'other' );
                    }
                }
                ...
                else if( 'tabulated' == $question->display_answers ) {
                ...
                $tabulate[$question->ID] = array( $answers_tabulated_cols, $results_tabulated_cols );
            }
        ...
        private function is_view_poll_results() {
            ...
            if( ( ( in_array( 'before', $this->view_results ) ) || ( in_array( 'after', $this->view_results ) && $is_voted ) || ( in_array( 'custom-date', $this->view_results ) && self::get_mysql_curent_date() >= $this->view_results_start_date ) || ( in_array( 'after-poll-end-date', $this->view_results ) && self::get_mysql_curent_date() >= convert_date( $this->poll_end_date, 'Y-m-d H:i:s' ) ) ) && ! in_array( 'never', $this->view_results ) && ( ( in_array( 'guest', $this->view_results_permissions ) && ! is_user_logged_in() ) || ( in_array( 'registered', $this->view_results_permissions ) && is_user_logged_in() ) )
            ) {
                return true;
            }
            return false;
        ...
        private static function display_poll_result_bar( $answer_id = 0, $procent = 0, $options = array(), $unique_id = '' ) {
            ...
            $result_bar .= ' style="height:' . intval( $options['bar_height'] + 2 * intval( $options['bar_border_width'] ) ) . 'px;" ';
            $result_bar .= '><div>';
        ...
        public function register_vote( $request ) {
            ...
            $SuperCookie  = $request['supercookie'];
            ...
            $this->update_poll_total_votes( $votes );
        ...
        public function return_poll_html(
            ...
            if( function_exists( 'icl_translate' ) ) {
                ...
                $this->singular_answer_result_votes_number_label = icl_translate( 'yop_poll', $poll_details->ID . '_singular_answer_result_votes_number_label', $this->singular_answer_result_votes_number_label );
                $this->plural_answer_result_votes_number_label   = icl_translate( 'yop_poll', $poll_details->ID . '_plural_answer_result_votes_number_label', $this->plural_answer_result_votes_number_label );
            ...
            if( in_array( 'custom-date', $this->view_results ) ) {
                if( $current_date >= $this->view_results_start_date ) {
                    if( $this->is_view_poll_results() ) {
                        $template = str_ireplace( '%POLL-ANSWER-RESULT-LABEL%', $this->answer_result_label, $template );
                    }
                }
            ...
            $template = str_ireplace( '%POLL-START-DATE%', esc_html( stripslashes( convert_date( $this->start_date, $this->date_format ) ) ), $template );
            $template = str_ireplace( '%POLL-PAGE-URL%', esc_html( yop_poll_kses( stripslashes( $this->poll_page_url ) ) ), $template );

            if( '18-01-2038 23:59:59' == convert_date( $this->poll_end_date, 'Y-m-d H:i:s' ) ) {
                $template = str_ireplace( '%POLL-END-DATE%', __yop_poll( 'Never Expire' ), $template );
            }
            else {
                $template = str_ireplace( '%POLL-END-DATE%', esc_html( stripslashes( convert_date( $this->end_date, $this->date_format ) ) ), $template );
            }
            ...
            foreach( $this->questions as $q ) {
                $poll_total_answers += $q->countanswers();
            }
